<?php
  ...
  $sql = "SELECT u.id, p.nombre, p.apellidos,p.imgperfil,u.usuario,u.id_tipo,u.status,p.correo,p.institucion,p.fecha FROM usuarios AS u INNER JOIN personal AS p ON u.id_personal=p.id WHERE u.id = '$idUsuario'";
    ...
    $row = $result->fetch_assoc();
            $query = "SELECT p.Reino,p.Id_hongo,p.status,p.Phyllum,p.Clase,p.Orden,p.Familia,p.Genero,p.Especie,p.ejemplar FROM usuarios AS u INNER JOIN hongos AS p ON u.id_personal=p.id WHERE p.status = 'I' and u.status='A'";

   $resultado=$mysqli->query($query);

    $Id_hongo=$_GET['Id_hongo'];
      $hongoss="SELECT p.Reino,p.Id_hongo,p.status,p.Phyllum,p.Clase,p.Orden,p.Familia,p.Genero,p.Especie,p.ejemplar,p.fecha,u.usuario,h.clave_regis,h.Emplear_tipo,m.Tip_vegetacion,m.Hospedero,c.Descripcion FROM usuarios AS u INNER JOIN hongos AS p ON u.id=p.id INNER JOIN herbario AS h ON h.id_hongo=p.Id_hongo INNER JOIN dat_eco AS m ON m.Id_hongo=p.Id_hongo INNER JOIN citado AS c ON c.Id_hongo=p.Id_hongo WHERE p.Id_hongo='$Id_hongo'";
  
  $resulta=$mysqli->query($hongoss);


    
      $hongogeo="SELECT p.municipio,l.localidad FROM datos_geograficos AS d INNER JOIN t_municipio AS p ON d.Municipio=p.id_municipio INNER JOIN t_localidad AS l ON d.Localidad=l.id_localidad WHERE d.Id_hongo = '$Id_hongo'";

  $resu=$mysqli->query($hongogeo);

    $img="SELECT i.id,h.Genero,h.Especie FROM hongos AS h INNER JOIN imagen AS i ON h.Id_hongo=i.id_hongo WHERE h.Id_hongo = '$Id_hongo'";

  $imghongo=$mysqli->query($img);   
 ...
 <?php while($row = $resulta->fetch_assoc()){ ?>
      ...
      <!--Card Panel-->
          ...
          <div id="card-panel-type" class="section">
            <h4 class="header"> <h4 capitalize>Observación <?php echo $Id_hongo; ?>: <?php echo $row['Genero']; ?> <?php echo $row['Especie']; ?></h4> Observación de <?php echo $row['usuario']; ?>
                ...
                <ul class="collection with-header">
                  ...
                  <li class="collection-item center tooltipped offset-s4 l2 offset-l1" data-position="top" data-delay="50" data-tooltip="fecha de la Observación"><?php echo $row['fecha']; ?></li>
                  <li class="collection-item center tooltipped offset-s4 l2 offset-l1" data-position="left" data-delay="50" data-tooltip="Reino">Reino: <?php echo $row['Reino']; ?></li>
                  <li class="collection-item center tooltipped offset-s4 l2 offset-l1" data-position="left" data-delay="50" data-tooltip="Phyllum">Phyllum: <?php echo $row['Phyllum']; ?></li>
                  <li class="collection-item center tooltipped offset-s4 l2 offset-l1" data-position="left" data-delay="50" data-tooltip="Clase">Clase: <?php echo $row['Clase']; ?></li>
                  <li class="collection-item center tooltipped offset-s4 l2 offset-l1" data-position="left" data-delay="50" data-tooltip="Orden">Orden: <?php echo $row['Orden']; ?></li>
                  <li class="collection-item center tooltipped offset-s4 l2 offset-l1" data-position="left" data-delay="50" data-tooltip="Familia">Familia: <?php echo $row['Familia']; ?></li>
                         <ul class="collapsible collapsible-accordion" data-collapsible="accordion">

                  <li>
                    <div class="collapsible-header center">Herbario</div>
                    <div class="collapsible-body">
                      <p>Herbario: <?php echo $row['Emplear_tipo']; ?></p>
       <p> Clave de registro: <?php echo $row['clave_regis']; ?></p>
                    </div>
                  </li>
                  <li>
                    <div class="collapsible-header center ">Datos Eco</div>
                     <div class="collapsible-body">
                      <p>Tipo de vegetacion: <?php echo $row['Tip_vegetacion']; ?></p>
       <p> Hospedero: <?php echo $row['Hospedero']; ?></p>
                    </div>
                  </li>
                  <li>
                    <div class="collapsible-header center">Citado</div>
                       <div class="collapsible-body">
                      <p>Libro: <?php echo $row['Descripcion']; ?></p>
       
                    </div>
                  </li>
                </ul>
                  <li class="collection-item left tooltipped offset-s4 l2 offset-l1" data-position="bottom" data-delay="50" data-tooltip="Observaciones"><?php echo $row['ejemplar']; ?></li>
            ...
            <div class="row">
              <!--  <a class="btn-floating  waves-effect waves-light right "><i class="mdi-content-add"></i></a> -->
              <div class="col s12 m8 l12">
              
                <div class="slider">
                  <ul class="slides">
                   <?php while($row = $imghongo->fetch_assoc()){ ?> <li>  
                      <img src="files/<?php echo $row['id'];?>.jpg" alt="no hay">
                      <!-- random image -->   
                      <div class="caption left-align">
                        <h5><?php echo $row['Genero']; ?></h5>
                        <h6 class="light grey-text text-lighten-3"> <?php echo $row['Especie']; ?></h6>
                        <h6 class="light grey-text text-lighten-3"> Oaxaca</h6>
                      </div>
                    </li>
                      <?php }?>
                ...
                <div class="row">
                  ...
                  <div class="col s12 m5 l4">
                    <div class="card-panel  green accent-4">
                      <span class="white-text"> <h5>Datos Geograficos  </h5>      Municipio:  <p class="flat-text">  <?php

                       while($row = $resu->fetch_assoc()){ ?>
                        <?php echo $row['municipio']; ?>
                            
</p>
                                           Localidad:  <p class="flat-text"><?php echo $row['localidad']; ?></p>
